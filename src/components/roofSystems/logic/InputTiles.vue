...
// import useValidation from '@/composables/use-Number';
...
// // Define the min and max values for the number range
// const slopeNum = ref(0);

// function validateRoofSlope() {
//     console.log(validateValue, errorMessage);
...
//     // }
// }
// const { validateValue, errorMessage } = useValidation(slopeModel);
...
const { errors, defineField } = useForm({
    validationSchema: toTypedSchema(
        v.object({
            slope: v.pipe(v.number(), v.slope('Invalid Slope'), v.nonEmpty('slope is required')),
            //   email: v.pipe(v.string(), v.email('Invalid Email'), v.nonEmpty('Email is required')),
            slopemin: v.pipe(v.number(), v.toMinValue(2, 'password too short'))
        })
    )
});
const [slope, slopeAttrs] = defineField('slope');
const [slopemin, slopeminAttrs] = defineField('slopemin');
</script>
<template>
    <div id="tile" class="flex flex-col w-full gap-2 bg-white shadow-lg shadow-cyan-800" style="margin-left: 10px">
        ...
        <div class="w-64 mt-6 ..." style="margin-left: 20px">
            ...
            <InputText id="slope" v-model="slopemin" type="text" placeholder="slope" v-bind="slopeminAttrs" :invalid="slope === null" @change="validateRoofSlope" />
        </div>
        <div>{{ errors.slopemin }}</div>
